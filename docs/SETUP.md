...
シミュレータではUIの表示確認ができますが、Magic Linkの完全な動作確認は難しいです。
...
Magic Linkの動作確認には実機でのテストを推奨します。
...
4. **Magic Linkのテスト**
   - メールアドレスを入力して「ログインリンクを送信」をタップ
   - メールアプリでMagic Linkを確認
   - リンクをタップするとExpo Goでアプリが開きログイン完了

> **Note**: 実機とPCが同じWi-Fiネットワークに接続されている必要があります。
